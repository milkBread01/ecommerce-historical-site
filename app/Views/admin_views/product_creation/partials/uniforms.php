...
<?php
    // Example uniform types - in a real application, this might come from a database or config file
    $uniformTypes = [
...
<div class="partial-section">
            ...
            <?php foreach ($uniformTypes as $category => $types): ?>
                <optgroup label="<?= esc(ucfirst($category)) ?>">
                    <?php foreach ($types as $value => $label): ?>
                        <option value="<?= esc($value) ?>" <?= old('uniform_type') === $value ? 'selected' : '' ?>><?= esc($label) ?></option>
                    <?php endforeach; ?>
                </optgroup>
            <?php endforeach; ?>
        </select>
    ...
    <div class="form-group">
        ...
        <select id="rank_insignia_present" name="rank_insignia_present">
            <option value="" <?= old('rank_insignia_present') === '' ? 'selected' : '' ?>>-- Select --</option>
            <option value="1" <?= old('rank_insignia_present') === '1' ? 'selected' : '' ?>>Yes</option>
            <option value="0" <?= old('rank_insignia_present') === '0' ? 'selected' : '' ?>>No</option>
        </select>
    </div>
    <div class="form-group">
        <label for="unit_patches_present">Unit Patches Present:</label>
        <select id="unit_patches_present" name="unit_patches_present">
            <option value="" <?= old('unit_patches_present') === '' ? 'selected' : '' ?>>-- Select --</option>
            <option value="1" <?= old('unit_patches_present') === '1' ? 'selected' : '' ?>>Yes</option>
            <option value="0" <?= old('unit_patches_present') === '0' ? 'selected' : '' ?>>No</option>
        </select>
    </div>
    <div class="form-group">
        <label for="buttons_original">Buttons Original:</label>
        <select id="buttons_original" name="buttons_original">
            <option value="" <?= old('buttons_original') === '' ? 'selected' : '' ?>>-- Select --</option>
            <option value="1" <?= old('buttons_original') === '1' ? 'selected' : '' ?>>Yes</option>
            <option value="0" <?= old('buttons_original') === '0' ? 'selected' : '' ?>>No</option>
        </select>
    </div>
    <div class="form-group">
